...
public abstract class AbstractRPCChannelHandler 
    ...
    private String generateChallenge() {
        ...
        currentChallenge = currentChallenge = Base64.getEncoder().encodeToString(challengeBytes);//DatatypeConverter.printBase64Binary(challengeBytes);
    ...
    private void authenticateResponse(String challenge, 
        ...
        byte[] expectedBytes = Base64.getDecoder().decode(expected); //DatatypeConverter.parseBase64Binary(expected);
        byte[] reponseBytes = Base64.getDecoder().decode(response); //DatatypeConverter.parseBase64Binary(response);
    ...
    private String generateResponse(String challenge) throws AuthException {
        ...
        try {
            mac.init(signingKey);
            byte[] output = 
                    mac.doFinal(Base64.getDecoder().decode(challenge));//DatatypeConverter.parseBase64Binary(challenge));
            return Base64.getEncoder().encodeToString(output);//DatatypeConverter.printBase64Binary(output);            
